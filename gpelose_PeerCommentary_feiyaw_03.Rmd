...
# Peer Commentary
1. On line 41, you have table(height). However, I cannot find a variable that called "height". If you want to use directly refer to "height" column, you can try attach() and dattach(), which is in Module 8 or 9. Or waht you can do is:
```{r}
table(d$height)
height<-(d$height)
```
I think you did that on other variable, but miss the first one.

2.On line 51, you should have pop_sd <- function(x){sqrt(v_height)} instead of pop_sd <- function(x){sqrt(v)}. you never name other variable as v before. 

3. As creat and apply the function on each variable in different chunk. you can directly use the function you create on previous chunk. it would only work if you run the function chunk before you run the caculation chunk. Or just use "knit".
Or you can create a large function for those little functions and only call the large function.You can refer to my homework for the strcuture of teh function.

4.I see you use rnorm()function for question 4. It is not a correct way to do that since it would assume all the data is normal distribution even if they are not. That is why all your data shows normal distribution on Q-Q plot. Some of the variable is not. normal distribution. 

5.For question 5, you need to use sample(). Sample () do not need a for-loop before. you can just apply it.
for example, on line 349 
```{r}
k <- 1
n <- 30
s <- NULL
...
It is for question 6 that you need to take mutiple sample, you will then use for-loop.

7. The chunk 35 is a recursive and it would be really slow for the r to get results. Try a better way. Since you only need a mean of those sample, it can be 
...
s <- NULL
for (i in 1:99){
    m<- sample(d$height,size=30, replace=TRUE)
    s[i]<- mean(m)
}
s
...
6. It would be helpful to annotated each of your function inside the chunks. I lose the tract in some chunks since i do not understand what you want to do.

If you need any help, I would love to help you. my email is feiyaw@bu.edu.
